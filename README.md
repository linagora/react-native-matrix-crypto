...
## Sharing history with somebody you invite
...
```ts
import { discardScopeKey, shareScopeKey } from 'react-native-matrix-crypto'

// 1. Remove them, and let the removal land.
await yourHomeserverKick(scope, '@departing:example.org')

// 2. Then rotate. `false` means there was no key of ours to rotate.
const rotated = await discardScopeKey(scope)

// 3. The replacement is made by the next send, and goes only to who is left.
await shareScopeKey(scope, await yourRemainingMembers(scope))
```

**Remove first, rotate second, and the order is not a style choice.** No new key is made by `discardScopeKey`. The replacement is created at the next `shareScopeKey`, and that call shares it with the users _it_ names — so rotating first and sharing before the removal has landed hands the fresh key to the very person it was rotated away from.

**It rotates only this device's key.** Everyone else in the conversation encrypts with their own, and a departed party keeps reading theirs until each of them rotates too. For two people that is the whole of it; for a group it is one participant's share of the work, and a product that presents it as more is misdescribing what it did.

**It takes nothing back.** Everything they already received, they keep — messages and keys both. This bounds the future and cannot touch the past, and no call in this library or anywhere else can. A product offering the gesture owes its user that sentence.

**`false` is not a failure.** It means this device has not encrypted in that scope, so there is no key out there that came from here, and the next send creates a fresh session regardless. It is reported rather than swallowed because "the key was rotated" and "there was no key of ours to rotate" are different facts, and a caller asserting the first should not pass on the second.
...
**The current release is 0.4.0.** It makes the two trust decisions that bound this library's behaviour the product's: `decryptEvent` takes a sender trust requirement, and a machine holding a verified cross-signing identity shares room keys by identity rather than with every device. That sits on top of device verification by a scannable code in all three modes the protocol defines, with showing and scanning announced separately, encryption and decryption, short-string verification, cross-signing identities and recovery through server-side secret storage. The npm badge at the top of this file always names the version actually published; this sentence names what the tree it points at contains.
...
The `audit` job is `npm audit`, and it is deliberately not a `gate:*` script. Every gate above asserts something about this tree, so a green gate stays green until somebody changes the tree; the audit reads an advisory database that moves on its own and can turn red on a commit that passed yesterday. It reads the packed tarball installed into an empty directory rather than the checkout, because the workspace's `node_modules` holds every development tool here and none of it reaches anyone who runs `npm install react-native-matrix-crypto` — and because npm cannot read this workspace at all, `packages/example-app` depending on the library through yarn's `link:` protocol. It runs two audits at two thresholds: nothing at all is tolerated in this package's own dependencies, which are this repository's to bump, while the `react` and `react-native` peer tree fails only on `critical`, because a finding there is React Native's to fix and arrives through a version range the consumer supplies. Everything below the threshold is printed rather than swallowed. `yarn audit:deps` runs the same script on a laptop.
...
**Outbound, what a room key is shared with follows from what this machine holds.** A machine that holds a verified cross-signing identity of its own — acquired by `bootstrapCrossSigning`, by `createCrossSigningIdentity`, by recovery, or by self-verification — shares with the identity-based collection strategy, the one upstream recommends per the guidance of MSC4153: only devices signed by their owner's identity receive the key, and a user whose client has published no identity receives none. A machine that never acquired an identity keeps upstream's `AllDevices` strategy, which shares with every device its recipients have, including devices nobody has signed or verified, and which upstream marks not recommended. That split is not a choice this library could put anywhere else: the identity-based strategy refuses outright when _this_ machine holds no cross-signing identity, before it considers a single recipient, so the two halves are the two possible states of a machine, and a product moves between them by deciding whether its users carry identities.

**Inbound, `decryptEvent` takes a `senderTrustRequirement`, and the default is `'any'`**: this release still decrypts events from any device unless the caller asks for a tightened tier, because whether a product's users carry cross-signing identities is something the product knows and this library cannot. The two tightened tiers — `'identity_signed_or_legacy'` and `'identity_signed'` — refuse events from devices no identity vouches for, and refuse them as their own error kind, `'sender_not_trusted'`, distinct from `'unknown_device'`, which means the event's provenance is broken and nothing fixes it. What neither tightened tier consults is local trust: a verification — a compared string or a scanned code alike — sets _local_ trust, and a peer whose device carries no cross-signature is refused by every tier however carefully somebody verified it. A product whose users verify devices without cross-signing identities should stay on the default and gate on `senderVerification` instead; [Limits you must design around](#limits-you-must-design-around) is where reading it is set out.
